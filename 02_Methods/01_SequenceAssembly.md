# 01_Sequence Assembly
* Summary: QC and assemble sequences using BBMap and spades
* Platform: Ceres
  * /project/fsepru/kmou/FS19C/lane1/

1. Create text file `replace.sh` with the following commands:
...
3. Rename `*.fastq.gz` files within `/project/fsepru/kmou/FS19C/lane1/` to fit the `*_1.fastq.gz` or `*_2.fastq.gz` format using the following bash script. It will create a directory `linked`, take the `*fastq.gz` files in current directory, shorten the names from something like `1-H12-96-441FEC_S103_L001_R2_001.fastq.gz` to `1-H12-96-441FEC_2.fastq.gz`, and copy over the short-named `*.fastq.gz` files to `linked` directory while also linking the short-named `*.fastq.gz` files to the original long-named `*.fastq.gz` files. Save bash script as `renamefiles.sh` and run on Ceres.

  <details><summary>renamefiles.sh script</summary>
...
5. Copy polished fasta sequences from `linked` directory to new directory `polished_genomes_100X`.

6. Run this to get complete list of polished genomes:
  ```
